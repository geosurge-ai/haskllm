...
instance ToJSON LLMInteraction where
  toJSON (LLMInteraction iType prompt resp rawResp) =
    object
      [ "type" .= iType,
        "prompt" .= prompt,
        "response" .= resp,
...
instance ToJSON CardGenerationLog where
  toJSON (CardGenerationLog ts name genInt themeInt archInt strInt) =
    object
      [ "timestamp" .= ts,
        "card_name" .= name,
        "generation" .= genInt,
...
extractOriginalContext fullJson = case fullJson of
  Object obj -> do
    ...
    theme <- case [ content | Object o <- genPrompt, Just (String role) <- [KM.lookup "role" o], role == "user", Just (String content) <- [KM.lookup "content" o], "Theme word" `T.isInfixOf` content
                  ] of
    ...
    archetype <- case [ content | Object o <- genPrompt, Just (String role) <- [KM.lookup "role" o], role == "user", Just (String content) <- [KM.lookup "content" o], "Player archetype:" `T.isInfixOf` content
                      ] of
    ...
    powerLevel <- case [ content | Object o <- genPrompt, Just (String role) <- [KM.lookup "role" o], role == "user", Just (String content) <- [KM.lookup "content" o], "Target power level:" `T.isInfixOf` content
                       ] of
