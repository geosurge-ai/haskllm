  ...
  [CodeBlock _ t]
    ...
    | otherwise     ->
        case eitherDecodeStrict (TE.encodeUtf8 t) of
          Right v -> Right v
          Left e  -> Right (String t) -- be lenient
  other ->
...
bodyToCardValue = \case
  ...
  other -> Left "bodyToCardValue: expected a single DefinitionList at top-level"
